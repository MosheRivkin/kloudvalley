...
class KV<Schemas extends Record<string, z.ZodTypeAny>> {
	...
	private getParsed<T>(raw: string | null, schema: z.ZodType<T>, key: string): T {
		if (raw === null) {
			const { success, data } = schema.safeParse(undefined)
			if (!success) throw new Error(`KV Error: value for key "${String(key)}" is unset but schema does not allow it.\nplease mark its schema as optional or provide it with a default value`)
			return data as T
		}
		...
		const { success, data, error } = schema.safeParse(parsedJson)
		if (!success) {
			throw new Error(`KV Error: error parsing value for key "${String(key)}".\n${z.prettifyError((error))}`)
		}
		return data as T
	}
	...
	async get<K extends keyof Schemas>(key: K): Promise<z.infer<Schemas[K]>> {
		...
		return this.getParsed(raw, schema, key as string) as z.infer<Schemas[K]>
	}
	/**
	 ...
	 * @returns {Promise<{[K in keyof Schemas]: z.infer<Schemas[K]>}>} A promise that resolves to an object containing all key-value pairs.
	 */
	async getAll(): Promise<{
		[K in keyof Schemas]: z.infer<Schemas[K]>
	}> {
		// const result = {} as { [K in keyof Schemas]: z.infer<Schemas[K]> }
		const keys = Object.keys(this.schemas)
		const result = this.getMultiple(keys) as { [K in keyof Schemas]: z.infer<Schemas[K]> }
		return result
	}
	/**
	 ...
	 * @returns {Promise<{[Key in K]: z.infer<Schemas[Key]>}>} A promise that resolves to an object containing the requested key-value pairs
	 */
	async getMultiple<K extends keyof Schemas>(
		keys: K[],
	): Promise<{ [Key in K]: z.infer<Schemas[Key]> }> {
		const result = {} as { [Key in K]: z.infer<Schemas[Key]> }
		let rowResult = new Map<string, string | null>()
		// Process in batches of 100
		for (let i = 0; i < keys.length; i += 100) {
			const batchKeys = keys.slice(i, i + 100)
			const batchValues = await this.kv.get(batchKeys as string[])
			rowResult = new Map([...rowResult, ...batchValues])
		}

		for (const key of rowResult.keys()) {
			const rawValue = rowResult.get(key) ?? null
			const schema = this.getSchema(key)
			const parsedValue = this.getParsed(rawValue, schema, key) as z.infer<Schemas[K]>
			result[key as K] = parsedValue
		}
		return result
	}
}
